...
常规 Linux 发行版可以使用官方脚本安装，阿里和 azure 的 `docker-ce.repo` 文件里都是从官方源同步过来的，这个文件里地址还是指向官方，所以我们得加个sed修改下再执行
...
离线有两种方式，一种是导出离线 rpm 和 deb，这里只说 static bin 方式，官方不推荐 static bin，因为有 libcontainer 相关依赖还是需要包管理安装的。如果国产 Linux 发行版本以及离线方式可以尝试下。
...
去网站 `https://download.docker.com/linux/static/stable/` 里的 `x86_64/` 和 `aarch64/` 下载对应架构的，例如现在是 `docker-27.5.1.tgz`

```shell
wget https://download.docker.com/linux/static/stable/$(uname -m)/docker-27.5.1.tgz
# 国内镜像站
wget https://mirrors.aliyun.com/docker-ce/linux/static/stable/$(uname -m)/docker-27.5.1.tgz
```
