...
k8s 的 pod 默认开启 `enableServiceLinks`，开启下会把同一个 namespace 里所有的 service 和 service 的 port 按照一定规则生成环境变量传递给 pod 的容器，而 pm2 在启动时会导入系统中的环境变量，当环境变量数量过多时，就会报错`[PM2][ERROR] Process failed to launch spawn E2BIG`，内部测试了下 pm2 大概环境变量数量为大于等于 2050 的时候会报这个错误。
