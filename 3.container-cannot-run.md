...
这种情况看下镜像的`docker inspect coredns/coredns:1.1.3 -f '{{.Architecture}}'`, 执行下 arch 命令看看和镜像的架构是否对应得上，一般这个问题是镜像架构和系统的 cpu 架构不一致。不一致的话构建镜像的RUN阶段和起容器执行命令都会出现这个问题。也可能是容器里的二进制文件架构不对，可以把容器起来后 `docker cp` 到宿主机上，file 命令看下它的架构
